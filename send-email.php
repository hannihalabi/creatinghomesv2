...
$required_fields = ['name', 'email', 'serviceType', 'propertyArea', 'propertyAddress', 
                    'elevatorAvailability', 'addonsPreference'];
$errors = [];
...
try {
    ...
    if (isset($_FILES['projectImages']) && !empty($_FILES['projectImages']['name'][0])) {
        ...
        foreach ($_FILES['projectImages']['tmp_name'] as $key => $tmp_name) {
            if ($_FILES['projectImages']['error'][$key] === UPLOAD_ERR_OK) {
                $file_size = $_FILES['projectImages']['size'][$key];
                $file_type = $_FILES['projectImages']['type'][$key];
                $file_name = $_FILES['projectImages']['name'][$key];
